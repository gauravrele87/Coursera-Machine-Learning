...
J = sum(J(:)); %without regularization
 
%making the bias zero
Theta1reg = [zeros(size(Theta1,1),1) Theta1(:,2:end)]; 
Theta2reg = [zeros(size(Theta2,1),1) Theta2(:,2:end)]; 

%with regularization
J = J + (lambda/(2*m))*(sum(Theta1reg(:).*Theta1reg(:))+...
     sum(Theta2reg(:).*Theta2reg(:)));
...
delta2 = 0;
%for t=1:m
  %Step 1 calculate a3 a1 z2 a2 done above
  %step 2
  d3= a3-yvec;  
%step3
